import { COLS, ROWS, TILE, Rack, generateRacks, buildGrid, buildPool, generateSpots, MonitorPost } from './world';
import { sounds } from './sound';

export type DecideFn = (
  postId: number,
  action: 'spare' | 'purge' | 'flag',
) => Promise<{
  ...
  return {
    ...
    interact() {
      ...
      cb.onDialogOpen({
        ...
        topic: n.topic,
        custom: n.custom,
        flagsLeft: game!.flags,
        openedAt: performance.now(),
        timeLimit: DIALOG_LIMIT,
      });
      ...
      dialogTimeout = setTimeout(() => {
        ...
        decideFn(timedOut.id, 'flag')
          .then((verdict) => {
            const kind = verdict.kind;
            if (kind === 'ai') game!.escapedBots++;
            else game!.killedHumans++;
            timedOut.state = 'spared';
            timedOut.correct = false;
            timedOut.action = 'timeout';
            const tells = verdict.tells.length
              ? verdict.tells
              : [
                  kind === 'ai'
                    ? 'Community-submitted as AI — no fingerprint notes provided.'
                    : 'Community-submitted as human — no fingerprint notes provided.',
                ];
            cb.onRevealShow({
              verdict: 'TIME OUT',
              verdictClass: 'neon-mag',
    ...
    async decide(action: 'spare' | 'purge' | 'flag') {
      ...
      if (action === 'flag') {
        ...
        if (action === 'purge') {
          n.state = 'purged';
          n.targetScale = 0.82;
          burst(n.x, n.y, correct ? '#34e7ff' : '#ff3d7f');
        } else {
          n.state = 'spared';
          n.targetScale = 1;
        }
        if (correct) {
          ...
          bigClass = 'neon-green';
          deltaTxt = '+6 INTEGRITY';
          deltaClass = 'neon-green';
        } else {
          sounds.wrong();
          game.wrong++;
          game.streak = 0;
      ...
      if (correct) {
        ...
        cb.onRevealShow({
          ...
          delta: deltaTxt,
          deltaClass,
          tells: tells.length ? tells : [fallback],
        });
      }
    },
